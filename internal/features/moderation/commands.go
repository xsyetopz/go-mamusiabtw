...
func warn() commandapi.SlashCommand {
	return commandapi.SlashCommand{
		Name:          "warn",
...
func warnCreateCommand(locales []string, t commandapi.Translator) discord.ApplicationCommandCreate {
	maxLen := 255
	minLen := 1
...
func withTargetUserID(t commandapi.Translator, userID uint64) commandapi.Translator {
	t.UserID = userID
	return t
}
...
func warnAppendAudit(ctx context.Context, s commandapi.Store, guildID, actorID, targetID uint64, now time.Time) {
	if s == nil {
		return
	}
...
func unwarn() commandapi.SlashCommand {
	return commandapi.SlashCommand{
		Name:          "unwarn",
...
func unwarnCreateCommand(locales []string, t commandapi.Translator) discord.ApplicationCommandCreate {
	perm := discord.PermissionModerateMembers
