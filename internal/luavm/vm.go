...
func NewFromFile(fileName string, opts Options) (*VM, error) {
	...
	// Host loads the plugin script from disk. Plugin code itself gets no filesystem API.
...
func (v *VM) registerHostAPI() {
	...
	t.RawSetString("log", v.L.NewFunction(v.luaLog))
	t.RawSetString("include", v.L.NewFunction(v.luaInclude))
	t.RawSetString("t", v.L.NewFunction(v.luaT))
	t.RawSetString("kv_get", v.L.NewFunction(v.luaKVGet))
	t.RawSetString("kv_put", v.L.NewFunction(v.luaKVPut))
	t.RawSetString("kv_del", v.L.NewFunction(v.luaKVDel))
