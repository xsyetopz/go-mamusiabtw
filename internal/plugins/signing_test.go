...
func TestReadTrustedKeysFileAndLoadTrustedKeys(t *testing.T) {
	...
	payload := plugins.TrustedKeys{
		Keys: []plugins.TrustedKey{
			{
				KeyID:        "file-key",
	...
	readKeys, err := plugins.ReadTrustedKeysFile(filePath)
	if err != nil {
	...
	loadedKeys, err := plugins.LoadTrustedKeys(
		context.Background(),
		filePath,
...
func TestVerifyDirSignatureAndHashDir(t *testing.T) {
	...
	hashBefore, err := plugins.HashDir(dir)
	if err != nil {
	...
	hashAfter, err := plugins.HashDir(dir)
	if err != nil {
	...
	if err := plugins.VerifyDirSignature(dir, signature, keys); err != nil {
		t.Fatalf("VerifyDirSignature(valid): %v", err)
	}

	if err := plugins.VerifyDirSignature(dir, signature, map[string]ed25519.PublicKey{}); err == nil {
